...
"继续"这一步最大的摩擦是复制粘贴。Claude Code、Codex、Cursor 都支持 MCP，把"项目现场"和"续接上下文"做成两个 MCP 工具，AI 在对话里就能直接取，用户说一句"按 Working Corpus 里的现场继续做 T03"就行。连接器只读，不调用模型，不改数据。
...
| 版本 | 形态 |
|---|---|
| MVP（已完成） | 本地网页 + 命令行 + MCP 连接器，MCP 在 Claude Code 和 Codex 里实测可用 |
