...
<sub>截图是示例数据：AI 两次说导出做好了，中间用户报告过文件打不开。Working Corpus 把它标成"待验证"，不算完成。页面跟随系统切换深色和浅色。</sub>
...
| 区块 | 回答什么 |
|---|---|
| 语料带 | 一整片像素格，每个方块是一条消息，颜色是它支撑的任务的状态；一眼看出多少对话已经变成了结论 |
...
| 最近决定 | 采用了什么、否决了什么；被新决定推翻的标为"已被替代"；原文没说原因就写"未说明" |
...
- **跨工具。** Claude Code、Codex（命令行和 VS Code 扩展都算）、VS Code 自带聊天的会话直接读本机文件，网页 AI 的对话粘贴导入或从 Google Takeout 导入。同一件事在几个工具里做过，只算一个任务。
...
网页 AI 的对话在"接入设置"里粘贴导入，可以附上原页面链接；Gemini 可以装浏览器扩展，在对话页面上点一下就加入项目（见 `docs/EXTENSION.md`）。用"你：""Gemini："这样的开头区分发言者；只复制了一部分就勾上"只复制了一部分"，Working Corpus 不会拿缺失的前文下结论。Gemini 的历史也可以从 Google Takeout 导出后批量导入，直接选 zip 包，先列出所有对话，只挑和项目有关的。
...
| `docs/EXTENSION.md` | Gemini 浏览器扩展：手动加载、边界、验证过什么 |
